...
def mock_vector_db():
    ...
    mock_db = Mock()
    # Mock query_points which is the actual API used
    mock_response = MagicMock()
    mock_response.points = []
    mock_db.query_points = MagicMock(return_value=mock_response)
    return mock_db
...
class TestGraphAwareRetriever:
    ...
    async def test_retrieve_empty_vector_results(
        ...
        # Mock empty vector search (query_points returns response.points = [])
        mock_response = MagicMock()
        mock_response.points = []
        mock_vector_db.query_points = MagicMock(return_value=mock_response)
        ...
        assert results == []
        mock_vector_db.query_points.assert_called_once()
    ...
    async def test_score_path_with_expert_weights(self, retriever_config):
        ...
        score = await retriever._score_path(path, expert_type="LiteralExpert")
    ...
    async def test_score_path_no_expert(self, retriever_config):
        ...
        score = await retriever._score_path(path, expert_type=None)
...
class TestRetrieverIntegration:
    ...
    async def test_retrieve_with_mock_vector(
        ...
        # Mock vector search to return our test chunk via query_points
