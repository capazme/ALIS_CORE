...
class TestLegalKnowledgeGraphInterpret:
    ...
    @pytest.mark.asyncio
    async def test_interpret_with_synthesis_mode(self, mock_kg):
        """interpret() con sintesi divergent mode."""
        # Mock SynthesisResult
        mock_synthesis = MagicMock()
        mock_synthesis.synthesis = "Divergent synthesis"
        mock_synthesis.mode = MagicMock()
        mock_synthesis.mode.value = "divergent"
        mock_synthesis.expert_contributions = {"precedent": {"interpretation": "Test"}}
        mock_synthesis.combined_sources = []
        mock_synthesis.confidence = 0.95
        mock_synthesis.disagreement_analysis = MagicMock()
        mock_synthesis.disagreement_analysis.to_dict.return_value = {"has_disagreement": True}
        mock_synthesis.alternatives = [{"expert": "literal", "position": "Alternative view"}]
        mock_synthesis.explanation = "Experts disagree on interpretation"
        mock_synthesis.trace_id = "divergent_123"
        mock_synthesis.execution_time_ms = 80.0
        ...
        mock_orchestrator = AsyncMock()
        mock_orchestrator.process_with_routing = AsyncMock(
            return_value=(mock_synthesis, mock_routing)
        )
        mock_kg._orchestrator = mock_orchestrator

        result = await mock_kg.interpret("Orientamento Cassazione")
