...
class TestPrepareBridgeMappings:
    ...
    def test_primary_mappings(self, sample_article):
        ...
        # Parse and chunk
        from merlt.pipeline.parsing import parse_article
        from merlt.pipeline.chunking import chunk_article
...
class TestIngestArticleWithoutGraph:
    ...
    async def test_ingest_creates_chunks(self, sample_article):
        ...
        assert result.chunks[0].urn.endswith("-com1")
        assert result.chunks[1].urn.endswith("-com2")
