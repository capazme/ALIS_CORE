...
# ============================================================================
# NormTree and related classes for hierarchical tree structure
# Copied from merlt/clients/visualex_client.py for compatibility
# ============================================================================
...
async def get_hierarchical_tree(url: str) -> tuple[NormTree, int]:
    ...
    This function uses the existing get_tree() function and converts
    the flat article list into a NormTree structure.
    ...
    """
    # Use existing get_tree function
    result = await get_tree(url, link=True, details=True, return_metadata=False)
    
    # Handle error case (get_tree returns error string as first element)
    if isinstance(result, tuple) and len(result) == 2:
        articles, count = result
        if isinstance(articles, str):  # Error message
            return articles, 0
    else:
        return "Invalid response from get_tree", 0
    
    # Build tree from flat articles list
    tree = NormTree(urn=url)
    
    if not isinstance(articles, list):
        return "Invalid articles format", 0
    
    for art in articles:
        # Extract article info from the dict structure
        if isinstance(art, dict):
            art_num = str(art.get("number", ""))
            art_title = art.get("title")
            art_position = art.get("position")
        else:
            # Fallback if structure is different
            art_num = str(art) if art else ""
            art_title = None
            art_position = None
