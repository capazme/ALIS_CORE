...
// Graph visualization content for Knowledge Graph explorer
...
type TabContent = NormaBlock | LooseArticle | ArticleCollection | GraphBlock;
...
interface AppState {
    ...
    addGraphTab: (rootUrn: string, label?: string, depth?: number) => string;
...
const appStore = createStore<AppState>()(
    persist(
        immer((set, get) => ({
            ...
            // Open graph visualization in a new workspace tab
            addGraphTab: (rootUrn, label, depth = 2) => {
                let tabId = '';
                set((state) => {
                    const tabCount = state.workspaceTabs.length;
                    const cascade = (tabCount % 5) * 40;

                    // Generate a smart label from URN if not provided
                    const smartLabel = label || (() => {
                        const match = rootUrn.match(/art(\d+)/i);
                        if (match) {
                            return `Grafo Art. ${match[1]}`;
                        }
                        return `Grafo ${rootUrn.slice(-20)}`;
                    })();

                    const newTab: WorkspaceTab = {
                        id: uuidv4(),
                        label: smartLabel,
                        position: { x: 150 + cascade, y: 120 + cascade },
...
// Export types
export type { WorkspaceTab, NormaBlock, LooseArticle, ArticleCollection, CollectionArticle, GraphBlock, TabContent, SearchPanelState };
