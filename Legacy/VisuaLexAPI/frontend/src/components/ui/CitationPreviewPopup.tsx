/**
 * CitationPreviewPopup - Popup DRAGGABLE per preview di citazioni normative.
 *
 * Mostra il contenuto dell'articolo citato con opzione per aprire in nuova tab.
 * Trascinabile liberamente su tutto lo schermo.
 */
...
interface CitationPreviewPopupProps {
  ...
  position: { top: number; left: number; startOffset: number; endOffset: number };
  targetElement: HTMLElement | null;  // Elemento DOM della citazione (per estrarre contesto)
  onClose: () => void;
  onOpenInTab: (citation: ParsedCitationData) => void;
  onMouseEnter?: () => void;
  onMouseLeave?: () => void;
  // NER feedback callbacks (opzionali)
  onConfirmCorrect?: (data: {
    originalText: string;           // Testo originale dallo span
    parsed: ParsedCitationData;
    targetElement: HTMLElement;     // Per estrarre contesto
  }) => void;
    ...
    <AnimatePresence>
      {isVisible && citation && (
        <motion.div
          ...
          className={cn(
            "fixed z-[200] w-[400px] max-w-[calc(100vw-24px)]",
        ...
        >
          <div className={cn(
            "bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden",
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                <GripVertical size={14} className="text-slate-400 dark:text-slate-500 shrink-0" />
            ...
            {/* Footer - Feedback NER + Apri in tab */}
            <div className="px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800">
              <div className="flex items-center justify-between gap-2">
                {/* Feedback buttons (solo se callbacks forniti e targetElement disponibile) */}
                {(onConfirmCorrect || onRequestCorrection) && targetElement && (
                  <div className="flex items-center gap-1">
                    {onConfirmCorrect && (
                      <button
                        onClick={() => {
                          if (!citation || !targetElement) return;
                        ...
                        disabled={isLoading}
                        className="flex items-center gap-1 px-2 py-1 text-xs text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        title="Conferma citazione corretta"
                      >
                        <Check size={14} />
                        Corretto
                      </button>
                    ...
                    {onRequestCorrection && (
                      <button
                        ...
                        disabled={isLoading}
                        className="flex items-center gap-1 px-2 py-1 text-xs text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        title="Correggi citazione"
                      >
                        <X size={14} />
                        Correggi
                      </button>
