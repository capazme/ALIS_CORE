...
import { wrapCitationsInHtml, type ExtractCitationsOptions } from '../../../utils/citationMatcher';
...
interface BrocardiSectionProps {
  ...
  /** Norma corrente per il context-aware citation detection */
  currentNorma?: { tipo_atto: string; data?: string; numero_atto?: string };
  /** Se true, abilita il rilevamento di norme senza articolo (mostra art. 1) */
  includeNormsWithoutArticle?: boolean;
}

function BrocardiSectionContent({ title, content, icon, currentNorma, includeNormsWithoutArticle = true }: BrocardiSectionProps) {
  ...
  // Apply citation wrapping with context-aware detection
  const citationOptions: ExtractCitationsOptions = {
    defaultNorma: currentNorma,
    includeNormsWithoutArticle,
    maxContextDistance: 400, // Brocardi sections can have longer contexts
  };

  const wrapWithCitations = (html: string) => {
    try {
      return wrapCitationsInHtml(html, citationOptions);
    } catch (e) {
      console.warn('[BrocardiSection] Citation wrapping failed:', e);
      return html;
    }
  };
          ...
          {Array.isArray(validContent) ? (
            <ul className="divide-y divide-slate-100 dark:divide-slate-700/50">
              {validContent.map((item, idx) => {
                ...
                const processedItem = wrapWithCitations(String(item || ''));
                return (
                  <li key={uniqueKey} className="p-4 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                    <SafeHTML html={processedItem} className="inline leading-relaxed" />
                  </li>
                );
              })}
            </ul>
            ...
            <SafeHTML
              html={wrapWithCitations(String(validContent || ''))}
...
function RelazioniSection({
  ...
  // Citation wrapping helper
  const wrapWithCitations = (text: string): string => {
    try {
      return wrapCitationsInHtml(text, {
        defaultNorma: currentNorma,
        includeNormsWithoutArticle: true,
        maxContextDistance: 400,
      });
    } catch (e) {
      return text;
    }
        ...
        <div className="divide-y divide-amber-100/50 dark:divide-amber-900/20">
          {relazioni.map((rel, idx) => (
            <div key={`relazione-${idx}`} className="p-4 hover:bg-amber-50/30 dark:hover:bg-amber-900/10 transition-colors">
              ...
              <SafeHTML
                html={wrapWithCitations(rel.testo)}
...
function RelazioneCostituzione({
  relazione,
  currentNorma,
}: {
  relazione: NonNullable<BrocardiInfoType['RelazioneCostituzione']>;
  currentNorma?: { tipo_atto: string; data?: string; numero_atto?: string };
}) {
  const [isOpen, setIsOpen] = useState(false);
    ...
    <div className="card bg-white dark:bg-slate-800 border border-purple-200/60 dark:border-purple-900/40 shadow-sm rounded-xl mb-3 overflow-hidden transition-all hover:shadow-md">
      ...
      {isOpen && (
        <div className="p-5 hover:bg-purple-50/10 dark:hover:bg-purple-900/5 transition-colors">
          ...
          <SafeHTML
            html={wrapWithCitations(relazione.testo)}
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-700/50">
          {footnotes.map((fn, idx) => (
            <div key={`footnote-${fn.numero || idx}`} className="p-3 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors flex items-start gap-3 group">
              ...
              />
              <SafeHTML
                html={wrapWithCitations(fn.testo)}
                className="text-sm text-slate-600 dark:text-slate-300 flex-1 leading-relaxed"
              />
            </div>
          ))}
        </div>
    ...
    <div className="brocardi-display bg-slate-50/50 dark:bg-slate-800/30 rounded-xl p-4 sm:p-5 border border-slate-100 dark:border-slate-800">
      ...
      {isMainOpen && (
        <div className="space-y-4 animate-in slide-in-from-top-2 fade-in duration-300">
          ...
          {info.Massime && info.Massime.length > 0 && (
            <MassimeSection massime={info.Massime} currentNorma={currentNorma} />
          )}
          ...
          {info.Relazioni && info.Relazioni.length > 0 && (
            <RelazioniSection relazioni={info.Relazioni} onArticleClick={onArticleClick} currentNorma={currentNorma} />
          )}

          {/* Constitution relation (Meuccio Ruini) */}
          {info.RelazioneCostituzione && (
            <RelazioneCostituzione relazione={info.RelazioneCostituzione} currentNorma={currentNorma} />
          )}
