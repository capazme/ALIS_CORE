...
import { ExternalLink, Zap, FolderPlus, Copy, StickyNote, Highlighter, Share2, Download, X, MoreHorizontal, Clock, BookOpen, GitCompare, Brain } from 'lucide-react';
...
import { ProposeEntityDrawer } from '../merlt/ProposeEntityDrawer';
import { ProposeRelationDrawer } from '../merlt/ProposeRelationDrawer';
...
export function ArticleTabContent({ data, onCrossReferenceNavigate, onOpenStudyMode }: ArticleTabContentProps) {
    ...
    // MERL-T Knowledge Graph state
    const [showMerltPanel, setShowMerltPanel] = useState(false);
    const [showProposeEntityDrawer, setShowProposeEntityDrawer] = useState(false);
    const [showProposeRelationDrawer, setShowProposeRelationDrawer] = useState(false);
    const [selectedTextForProposal, setSelectedTextForProposal] = useState<SelectedTextInfo | null>(null);
    const merltStatus = useMerltArticleStatus({
    ...
    // NER Feedback state
    const [showCitationCorrectionCard, setShowCitationCorrectionCard] = useState(false);
    const [citationCorrectionData, setCitationCorrectionData] = useState<{
        selectedText: string;
        targetElement?: HTMLElement;  // Per estrarre contesto dal DOM
        originalParsed?: any;
    ...
    // Handler for SelectionPopup propose entity action (R2)
    const handlePopupProposeEntity = (info: SelectedTextInfo) => {
        if (!isAuthenticated) {
            showToast('Effettua il login per proporre entità', 'error');
            return;
        }
        setSelectedTextForProposal(info);
        setShowProposeEntityDrawer(true);
    };

    // NER Feedback handlers
    const handleCitationConfirm = async (data: {
        originalText: string;
        parsed: any;
        targetElement: HTMLElement;
    }) => {
        try {
            await merltService.confirmCitation({
                article_urn: norma_data?.urn || '',
                text: data.originalText,
                parsed: data.parsed,
                user_id: user?.id || 'anonymous',
            });
            showToast('Citazione confermata!', 'success');
        } catch (error) {
            console.error('Failed to confirm citation:', error);
            showToast('Errore durante la conferma', 'error');
        }
    };

    const handleCitationCorrection = (data: {
        originalText: string;
        parsed?: any;
        targetElement: HTMLElement;
        confidenceBefore?: number;
    }) => {
        // Get anchor position from the target element
        let anchorPosition = { top: 200, left: 100 };
        ...
        setCitationCorrectionData({
            selectedText: data.originalText,
            targetElement: data.targetElement,
            originalParsed: data.parsed,
            confidenceBefore: data.confidenceBefore,
            source: 'citation_preview',
            anchorPosition,
        });
        setShowCitationCorrectionCard(true);
    };

    const handleAnnotateCitation = (info: { text: string; startOffset: number; endOffset: number }) => {
        // Get anchor position from current selection
        const selection = window.getSelection();
        let anchorPosition = { top: 200, left: 100 };
        ...
        // Per annotation da selezione manuale, non abbiamo targetElement
        // Il contesto verrà estratto dal testo dell'articolo
        setCitationCorrectionData({
            selectedText: info.text,
            // targetElement non disponibile per selezioni manuali
        ...
        <div className="animate-in fade-in duration-300 relative">
            ...
            <div className="glass-toolbar sticky top-0 z-10 flex items-center justify-between p-2 rounded-t-xl mb-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-b-2 border-slate-200/50 dark:border-slate-800/50">
                ...
                <div className="flex md:hidden items-center gap-1">
                    ...
                    {/* MERL-T Knowledge Graph button */}
                    <button
                        onClick={() => setShowMerltPanel(true)}
                        className={cn(
                            "p-2 lg:p-2.5 rounded-lg transition-colors relative",
                            merltStatus.graphStatus?.exists
                                ? "text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20"
                                : "text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-primary-500"
                        )}
                        title="Knowledge Graph"
                    >
                        <Brain size={20} />
                        {merltStatus.pendingCount > 0 && (
                            <span className="absolute -top-1 -right-1 w-4 h-4 bg-amber-500 text-white text-[10px] rounded-full flex items-center justify-center font-medium">
                                {merltStatus.pendingCount > 9 ? '9+' : merltStatus.pendingCount}
                            </span>
                        )}
                    </button>
                    {url && (
                ...
                <div className="hidden md:flex items-center gap-1">
                    ...
                    <button
                        ...
                        title={merltStatus.graphStatus?.exists ? "Nel Knowledge Graph" : "Contribuisci al Knowledge Graph"}
                    >
                        <Brain size={16} />
                        {merltStatus.pendingCount > 0 && (
            ...
            <div className="relative group/content" ref={contentRef}>
                ...
                {/* Brocardi Display - inside contentRef so citation hover handlers work */}
                {brocardi_info && (
            ...
            <CitationPreviewPopup
                ...
                onConfirmCorrect={handleCitationConfirm}
                onRequestCorrection={handleCitationCorrection}
            ...
            {/* MERL-T Knowledge Graph Inspector Panel */}
            <MerltInspectorPanel
                ...
                articolo={norma_data.numero_articolo}
                isLoading={merltStatus.isLoading}
                isValidating={merltStatus.isValidating}
                isEnriching={merltStatus.isEnriching}
                progressMessage={merltStatus.progressMessage}
                error={merltStatus.error}
                inGraph={merltStatus.graphStatus?.exists ?? false}
                entityCount={merltStatus.validatedCount}
                pendingEntities={merltStatus.pendingEntities}
                pendingRelations={merltStatus.pendingRelations}
                hasVotedEntity={merltStatus.hasVotedEntity}
                hasVotedRelation={merltStatus.hasVotedRelation}
                uncommittedVoteCount={merltStatus.uncommittedVoteCount}
                onValidateEntity={async (entityId, vote, comment) => {
                    await merltStatus.validateEntity(entityId, vote, comment);
                }}
                onValidateRelation={async (relationId, vote, comment) => {
                    await merltStatus.validateRelation(relationId, vote, comment);
                }}
                onRequestEnrichment={merltStatus.requestEnrichment}
                onCommitFeedback={merltStatus.commitFeedback}
                onRefresh={merltStatus.refresh}
                onProposeEntity={() => {
                    if (!isAuthenticated) {
                        showToast('Effettua il login per proporre entità', 'error');
                        return;
                ...
                onProposeRelation={() => {
                    if (!isAuthenticated) {
                        showToast('Effettua il login per proporre relazioni', 'error');
                        return;
                    }
                    setShowProposeRelationDrawer(true);
                }}
            />

            {/* MERL-T Propose Entity Drawer */}
            {user?.id && (
                <ProposeEntityDrawer
                    isOpen={showProposeEntityDrawer}
                    onClose={() => {
                        setShowProposeEntityDrawer(false);
                        setSelectedTextForProposal(null);
                    }}
                    onSuccess={(entity) => {
                        showToast(`Entità "${entity.nome}" proposta con successo!`, 'success');
                        merltStatus.refresh();
                        setSelectedTextForProposal(null);
                    }}
                    articleUrn={merltStatus.graphStatus?.node_id || `urn:nir:stato:${norma_data.tipo_atto.replace(/\s+/g, '.')}:art${norma_data.numero_articolo}`}
                    articleText={article_text}
                    userId={user.id}
                    ambito={norma_data.tipo_atto.includes('civile') ? 'civile' : norma_data.tipo_atto.includes('penale') ? 'penale' : 'generale'}
                    selectedTextInfo={selectedTextForProposal || undefined}
                />
            )}

            {/* MERL-T Propose Relation Drawer */}
            {user?.id && (
                <ProposeRelationDrawer
                    isOpen={showProposeRelationDrawer}
                    onClose={() => setShowProposeRelationDrawer(false)}
                    onSuccess={() => {
                        showToast('Relazione proposta con successo!', 'success');
                        merltStatus.refresh();
                    }}
                    articleUrn={merltStatus.graphStatus?.node_id || `urn:nir:stato:${norma_data.tipo_atto.replace(/\s+/g, '.')}:art${norma_data.numero_articolo}`}
                    userId={user.id}
                    availableEntities={merltStatus.pendingEntities}
                />
            )}

            {/* NER Correction Floating Card */}
            {citationCorrectionData && (
                <CitationCorrectionCard
                    isOpen={showCitationCorrectionCard}
                    onClose={() => {
                        setShowCitationCorrectionCard(false);
                        setCitationCorrectionData(null);
                    }}
                    anchorPosition={citationCorrectionData.anchorPosition}
                    containerRef={contentRef}
                    selectedText={citationCorrectionData.selectedText}
                    articleUrn={norma_data?.urn || ''}
                    originalParsed={citationCorrectionData.originalParsed}
                    confidenceBefore={citationCorrectionData.confidenceBefore}
                    source={citationCorrectionData.source}
                    userId={user?.id || 'anonymous'}
                    getContextWindow={() => {
                        // Estrai contesto dal DOM se abbiamo targetElement (citazione rilevata)
                        if (citationCorrectionData.targetElement) {
                            const context = getContextFromDOM(
                                citationCorrectionData.targetElement,
                                500,  // chars before
                                500,  // chars after
                                '.article-content'  // boundary container
                            );
                            return context.full;
                        }
                        // Fallback per selezioni manuali: cerca nel testo dell'articolo
                        const plainText = (article_text || '').replace(/<[^>]+>/g, '');
                        const selectedText = citationCorrectionData.selectedText;
                        const index = plainText.indexOf(selectedText);
                        if (index >= 0) {
                            const before = plainText.slice(Math.max(0, index - 500), index);
                            const after = plainText.slice(index + selectedText.length, Math.min(plainText.length, index + selectedText.length + 500));
                            return `${before}[${selectedText}]${after}`;
                        }
                        // Se non troviamo il testo, ritorniamo solo il testo selezionato
                        return `[${selectedText}]`;
                    }}
                    onSuccess={(response) => {
                        console.log('NER feedback submitted:', response);
                        showToast(`Feedback inviato! Buffer: ${response.buffer_size} esempi`, 'success');
                    }}
                />
            )}
