...
import { Highlighter, StickyNote, Copy, Search, X, Sparkles, BookOpen } from 'lucide-react';
...
interface SelectionPopupProps {
  ...
  onProposeEntity?: (info: SelectedTextInfo) => void;
  onAnnotateCitation?: (info: SelectedTextInfo) => void;
}
...
export function SelectionPopup({
  ...
  const handleAction = (action: 'highlight' | 'note' | 'copy' | 'search' | 'propose-entity' | 'annotate-citation') => {
    switch (action) {
      case 'highlight':
          ...
          <div className="flex items-center">
            ...
            {onProposeEntity && (
              <>
                <div className="w-px h-5 bg-slate-700" />
                <button
                  onClick={() => handleAction('propose-entity')}
                  className="p-2.5 hover:bg-slate-700 transition-colors flex items-center gap-1.5 text-sm"
                  title="Proponi entità per Knowledge Graph"
                >
            ...
            {onAnnotateCitation && (
              <>
                <div className="w-px h-5 bg-slate-700" />
                <button
                  onClick={() => handleAction('annotate-citation')}
                  className="p-2.5 hover:bg-slate-700 transition-colors flex items-center gap-1.5 text-sm"
                  title="Annota come citazione normativa"
                >
