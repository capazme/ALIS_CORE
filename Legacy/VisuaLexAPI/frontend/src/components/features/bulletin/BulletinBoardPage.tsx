...
import {
  ...
  Package, Lightbulb, Inbox, Send, Check, X as XIcon, AlertCircle, Network, Landmark,
} from 'lucide-react';
...
type TabType = 'explore' | 'my' | 'suggestions' | 'graph';
...
export function BulletinBoardPage() {
  ...
  // Tab state - Default to 'explore' (Ambienti)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-1">
            Esplora il Knowledge Graph e condividi ambienti con la community
          </p>
        </div>
        {activeTab !== 'graph' && (
          <button
      ...
      {/* GRAPH TAB - Knowledge Graph Explorer */}
      {activeTab === 'graph' && (
        <div className="space-y-6">
          {/* Graph */}
          <div className="relative">
            <KnowledgeGraphExplorer
              ...
              className="w-full"
              onArticleClick={(urn) => {
                // TODO: Navigate to article in reader
                console.log('Navigate to article:', urn);
              }}
            />
          </div>

          {/* Sezione Segnalazioni sotto il grafo */}
          <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
            <IssueList userId={user?.id || 'anonymous'} />
          </div>
        </div>
      )}
