...
import { X, Edit2, FolderPlus, Check, Plus, FileText, AlertCircle } from 'lucide-react';
              ...
              tab.content.map((item) => {
                if (item.type === 'norma') {
                  ...
                  return (
                    ...
                    >
                      <PluginSlot
                        name="graph-view"
                        props={{
                          rootUrn: item.rootUrn,
                          depth: item.depth,
                          userId: user?.id,
                        }}
                        fallback={
                          <div className="flex items-center gap-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 text-sm text-slate-500 h-full">
                            <AlertCircle size={16} className="text-amber-500" />
                            Il Knowledge Graph e' disponibile solo per utenti MERL-T abilitati.
                          </div>
                        }
                      />
                    </div>
                  ...
                  return (
                    <div
                      key={item.id}
                      className="flex items-center gap-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 text-sm text-slate-500"
                    >
                      <AlertCircle size={16} className="text-amber-500" />
                      Contenuto non supportato nella versione vanilla.
                    </div>
                  );
                }
              })
