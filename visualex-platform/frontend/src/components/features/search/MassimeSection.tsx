...
export function MassimeSection({ massime, currentNorma }: MassimeSectionProps) {
  const [isOpen, setIsOpen] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedYear, setSelectedYear] = useState<string>('all');
  const [expandedIds, setExpandedIds] = useState<Set<string>>(new Set());

  // Citation wrapping options
  const citationOptions: ExtractCitationsOptions = useMemo(() => ({
              ...
              >
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Massima text with citation detection */}
                    <SafeHTML
                      html={wrapWithCitations(displayText)}
