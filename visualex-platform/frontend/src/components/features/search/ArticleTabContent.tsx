...
export function ArticleTabContent({ data, onCrossReferenceNavigate, onOpenStudyMode }: ArticleTabContentProps) {
    ...
    // Emit article:viewed event for plugins
    useEffect(() => {
        if (norma_data.urn) {
    ...
    }, [norma_data.urn, uniqueArticleId, user?.id]);
        ...
        <div className="animate-in fade-in duration-300 relative flex">
            ...
            <div className="relative group/content flex gap-2" ref={contentRef}>
                ...
                <SelectionPopup
                    ...
                    urn={norma_data.urn}
                />
                <div className="article-content prose prose-lg dark:prose-invert max-w-none legal-prose prose-slate prose-headings:font-bold font-serif px-2 sm:px-4" id={`article-content-${itemKey}`}>
                    {processedContent ? (
                ...
                {/* Plugin Slot: content overlay (e.g., MERLT citation correction, entity proposals) */}
                <PluginSlot
