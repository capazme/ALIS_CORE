...
type AdminTab = 'users' | 'feedback' | 'environments' | 'plugins';
...
export function AdminPage() {
  ...
  // Form states
  const [newUser, setNewUser] = useState({ email: '', username: '', password: '', isAdmin: false, isMerltEnabled: false });
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewUser({ email: '', username: '', password: '', isAdmin: false, isMerltEnabled: false });
  ...
  const handleToggleMerlt = async (userId: string, currentIsMerltEnabled: boolean) => {
    try {
      setActionLoading(true);
      await adminService.updateUser(userId, { isMerltEnabled: !currentIsMerltEnabled });
      await loadUsers();
    } catch (err: any) {
      setError(err.message || 'Errore nell\'aggiornamento');
    } finally {
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                      {users.map((u) => (
                        <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <button
                              onClick={() => handleToggleMerlt(u.id, u.is_merlt_enabled)}
                              disabled={!merltEnabled}
                              className={cn(
                                'inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors',
                                u.is_merlt_enabled
                                  ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400'
                                  : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400',
                                !merltEnabled ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-80 cursor-pointer'
                              )}
                              title={merltEnabled ? 'Abilita accesso MERL-T' : 'MERL-T disabilitato a livello sistema'}
                            >
                              <Brain size={12} />
                              {u.is_merlt_enabled ? 'Abilitato' : 'Disattivo'}
                            </button>
                          </td>
                          <td className="px-6 py-4">
            ...
            fallback={
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8">
                <div className="text-center text-gray-500">
                  ...
                  <p className="text-sm">
                    I plugin come MERL-T forniscono qui dashboard e strumenti di amministrazione aggiuntivi.
                  </p>
                </div>
              </div>
            }
              ...
              {merltEnabled && (
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id="isMerltEnabled"
                    checked={newUser.isMerltEnabled}
                    onChange={(e) => setNewUser({ ...newUser, isMerltEnabled: e.target.checked })}
                    className="w-4 h-4 text-emerald-600 rounded border-gray-300 focus:ring-emerald-500"
                  />
                  <label htmlFor="isMerltEnabled" className="text-sm text-gray-700 dark:text-gray-300">
                    Abilita MERL-T per l'utente
                  </label>
                </div>
              )}
