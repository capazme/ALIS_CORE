...
export function useCitationPreview(options: UseCitationPreviewOptions = {}) {
  ...
  const [state, setState] = useState<CitationPreviewState>({
    ...
    citation: null,
    article: null,
    position: { top: 0, left: 0, startOffset: 0, endOffset: 0 },
    targetElement: null,
  });
  ...
  const calculatePosition = useCallback((
    ...
    return { top, left, startOffset, endOffset };
  }, []);
  ...
  const showPreview = useCallback((
    ...
    hoverTimeoutRef.current = window.setTimeout(async () => {
      ...
      // Calcola posizione
      const position = calculatePosition(element, startOffset, endOffset);
  ...
  const updatePosition = useCallback(() => {
    if (state.targetElement) {
      const position = calculatePosition(
        state.targetElement,
        state.position.startOffset,
        state.position.endOffset
      );
      setState(prev => ({ ...prev, position }));
    }
  }, [state.targetElement, state.position.startOffset, state.position.endOffset, calculatePosition]);
