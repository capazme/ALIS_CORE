...
async def parse_article_input(article_string, normurn):
    ...
    for part in parts:
        ...
        if range_match:
            ...
            # Regex per verificare se la parte è un articolo con estensione (es. 1-bis, 2-ter, 168bis)
            # Accetta sia con trattino (1-bis) che senza (168bis)
            single_article_match = re.match(r'^(\d+(-?[a-z]+)?)$', part, re.IGNORECASE)
            if single_article_match:
                logging.debug(f"Found single article: {part}")
                # Normalizza: se ha estensione senza trattino, aggiungi il trattino
                # Es: "168bis" -> "168-bis" per coerenza con URN generator
                normalized_part = re.sub(r'^(\d+)([a-z]+)$', r'\1-\2', part, flags=re.IGNORECASE)
                if normalized_part != part:
                    logging.debug(f"Normalized article format: {part} -> {normalized_part}")
                articles.append(normalized_part)
            else:
