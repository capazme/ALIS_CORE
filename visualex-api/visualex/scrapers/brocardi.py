...
from ..utils.map import BROCARDI_CODICI
from ..models.norma import NormaVisitata
from ..utils.text_op import normalize_act_type
from ..utils.sys_op import BaseScraper
from ..utils.cache_manager import get_cache_manager
from ..utils.http_client import http_client
from ..exceptions import DocumentNotFoundError
from ..utils.selectors import BrocardiSelectors
...
class BrocardiScraper(BaseScraper):
    ...
    async def do_know(self, norma_visitata: NormaVisitata) -> Optional[Tuple[str, str]]:
        ...
        # Normalize the act type for matching
        norma = norma_visitata.norma
        tipo_norm = normalize_act_type(norma.tipo_atto_str, True, 'brocardi').lower()
        numero_atto = norma.numero_atto

        log.debug(f"Searching Brocardi for tipo={tipo_norm}, numero={numero_atto}")

        for txt, link in self.knowledge[0].items():
            txt_lower = txt.lower()
            # Match by act type first
            if tipo_norm in txt_lower:
                # If we have a number, also check it matches
                if numero_atto:
                    # Check if the number appears in the key (e.g., "n. 262")
                    if f"n. {numero_atto}" in txt_lower or f"n.{numero_atto}" in txt_lower:
                        log.info(f"Knowledge found for norma: {norma_visitata} -> {txt}")
                        return txt, link
                else:
                    # No number specified, just match by type
                    log.info(f"Knowledge found for norma: {norma_visitata} -> {txt}")
                    return txt, link

        # Fallback: try simple substring match with just the type
        for txt, link in self.knowledge[0].items():
            if tipo_norm in txt.lower():
                log.info(f"Knowledge found (fallback) for norma: {norma_visitata} -> {txt}")
                return txt, link
