...
export function QueryMonitorDashboard() {
  ...
  const getStatusBadge = (status?: QueryStatus) => {
    ...
    return (
      <Badge variant={config.variant} className="flex items-center" aria-label={`Status: ${config.label}`}>
  ...
  return (
    <div className="space-y-6" role="region" aria-label="Query Monitor Dashboard">
      {/* Header */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Query Monitor</h1>
          <p className="text-slate-400 mt-1">Real-time orchestration pipeline monitoring</p>
        </div>
        <Button onClick={() => refetch()} variant="outline" className="flex items-center gap-2 focus-visible:ring-2 focus-visible:ring-blue-500">
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            <div className="flex items-center justify-center py-8 text-slate-400" role="status" aria-label="Loading queries">
              <Loader className="w-6 h-6 animate-spin mr-2" aria-hidden="true" />
              Loading queries...
            </div>
          ) : queries.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full" aria-label="Query history">
                <thead>
                  <tr className="border-b border-slate-700">
                    <th scope="col" className="text-left py-3 px-4 text-slate-400 font-medium">Trace ID</th>
                    <th scope="col" className="text-left py-3 px-4 text-slate-400 font-medium">Query</th>
                    <th scope="col" className="text-left py-3 px-4 text-slate-400 font-medium">Status</th>
                    <th scope="col" className="text-left py-3 px-4 text-slate-400 font-medium">Confidence</th>
                    <th scope="col" className="text-left py-3 px-4 text-slate-400 font-medium">Timestamp</th>
                    <th scope="col" className="text-left py-3 px-4 text-slate-400 font-medium">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {queries.map((query) => (
  ...
  return (
    <tr className="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
      ...
      <td className="py-3 px-4 max-w-md">
        <div className="truncate text-slate-300">{query.query_text}</div>
      </td>
      <td className="py-3 px-4">{getStatusBadge(currentStatus as QueryStatus)}</td>
      <td className="py-3 px-4">
        {query.confidence ? (
          <span className="text-slate-300">{(query.confidence * 100).toFixed(1)}%</span>
        ) : (
      ...
      <td className="py-3 px-4">
        <Button onClick={onViewDetails} variant="ghost" size="sm" className="flex items-center gap-1 focus-visible:ring-2 focus-visible:ring-blue-500" aria-label={`View details for trace ${query.trace_id}`}>
